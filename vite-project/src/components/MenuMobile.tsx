...
export function MenuMobile() {
  const { lang, isMenuOpen } = useMobile()
  ...
  for (const [key, value] of Object.entries(allPhone)) {
    phoneBrandAndModels.push(
      <div className={classes.model}>
        <h4 className={`${classes.brandName} brand`} data-brandActive='false'>{key}</h4>
        {value.map((name: string) => (
          <div className={classes.modelName} data-brandActive='false'>{name}</div>
        ))}
      </div>
    )
  }
  ...
  return (
    <div className={`${classes.menu} ${isMenuOpen ? classes.active : ''}`}>
      ...
      <div className={`${classes.favourites} ${classes.menuItem}`}>
      ...
      <div className={`${classes.terms} ${classes.menuItem}`}>
      ...
      <div className={`${classes.contacts} ${classes.menuItem}`}>
      ...
      <div className={`${classes.langChose} ${classes.menuItem}`}>
        ...
        <div className={`${lang === 'en' ? classes.active : classes.lang}`}>Eng</div>
        <div className={`${lang === 'ru' ? classes.active : classes.lang}`}>Рус</div>
      </div>
    </div>
  )
}
...
// function closeOtherBrand(): void {
//   const allBrand =  document.querySelectorAll('brand')
//   allBrand.forEach
// }